...
app.use((error, req, res, next) => {
  // ExpressJS default error handler. Special middleware function with 4 parameters instead of 3 (error). ExpressJS treats middleware functions with 4 parameters as special/error handling functions. Only executed on requests that have an error attached to it.
  if (req.file) {
    // Multer adds a file property to the req object, so we can check if we do have a file attached to the request, and roll back the saving process/delete the file if we don't
    fs.unlink(req.file.path, (err) => {
      // We use the file system (fs) object to delete(unlink) the file. path is a property on the file object added by multer and it points to the file to be deleted. The callbac function will be triggered when the deletion is done
      console.log(err);
    });
  }
...
mongoose
  .connect(
    `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASSWORD}@academind-mongodb.r4go223.mongodb.net/`
  )
  .then(() => {
