...
const signup = async (req, res, next) => {
  ...
  try {
    ...
    // If the try block fails, we simply catch the error with a staus code of 500
  ...
  let hashedPassword;
  try {
    // hash() returns a promise and since we're already in an async function, it makes sense to just use an await on it
...
const login = async (req, res, next) => {
  ...
  try {
    ...
    // If the try block fails, we simply catch the error
    const error = new HttpError('Login failed, please try again', 500);
    return next(error);
  }

  if (!existingUser) {
    const error = new HttpError('Invalid credentials, could not log you in', 403); // 403 is a forbidden error
    return next(error);
  }
  ...
  try {
    token = jwt.sign({ userId: existingUser.id, email: existingUser.email }, process.env.JWT_KEY, {
      expiresIn: '1h',
    }); // We make sure to use the same private key as the one in signup route so we don't generate different tokens, so we can verify them on the backend
  } catch (err) {
